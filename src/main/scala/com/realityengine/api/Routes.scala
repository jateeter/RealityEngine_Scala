...
class Routes(
    ...
    engine.getAllMachines.find(_.name == machineName) match {
      ...
      case Some(machine) =>
        ...
        val inputVectorCount = inputSeqs.asArray
          ...
          .flatMap(h => h.hcursor.downField("events").as[Vector[Json]].toOption)
          .map(_.length)
          .getOrElse(0)
  ...
  private def renderPrometheusMetrics(): String = {
    ...
    val totalVecs   = stats.get[Int]("totalEvents").getOrElse(0)
        ...
        pathPrefix("machines") {
          concat(
            ...
            path(Segment / "process") { id => post { entity(as[Json]) { body =>
              val vec = body.hcursor.downField("inputEvent").as[Vector[Double]].getOrElse(Vector.empty)
