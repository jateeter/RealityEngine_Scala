...
# Load the corpus the deployment selected, not whatever MACHINES_DIR happens to
# hold. Under the deployment gate the agent exports MACHINES_DIR as the full
# RealityEngine_Machines repo, so this suite was booting 1,328 machines to test
# three it POSTs itself — 30s of startup for nothing, which then failed a 10s
# readiness budget and read as "Reality Engine did not become ready".
#
# startUniverse stamps the corpus it materialised; prefer that. Falling back to
# the inherited value keeps a bare local run working.
if [ -z "${MACHINES_DIR:-}" ] || [ ! -d "${MACHINES_DIR:-}" ] || [ -n "${RE_E2E_USE_ACTIVE_CORPUS:-1}" ]; then
  _sel="${CI_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../RealityEngine_CI" 2>/dev/null && pwd)}/.universe-engine-selection"
  if [ -f "$_sel" ]; then
    _active="$(sed -n 's/^MACHINE_CORPUS_ACTIVE_DIR=//p' "$_sel" | tail -1)"
    if [ -n "$_active" ] && [ -d "$_active/machines" ]; then
      MACHINES_DIR="$_active/machines"
      export MACHINES_DIR
      echo "  Machine corpus:         $MACHINES_DIR ($(find "$MACHINES_DIR" -name '*.json' | wc -l | tr -d ' ') machines)"
    fi
  fi
fi
...
# Wait for a service to answer.
#
# The budget is modest on purpose. With the corpus this suite should be running
# against, the Reality Engine answers in ~2s:
#
#   full corpus (1,328 machines)      ready in 30s
#   regression corpus (20 machines)   ready in  2s
#
# An earlier fix raised this to 120s because the engine was taking 30 seconds —
# which accommodated a bug rather than fixing it. A two-minute timeout on a
# service that should answer in two seconds is how the *next* real hang goes
# unnoticed for two minutes. The corpus below is the actual fix; this budget is
# back to something that fails usefully.
wait_for_http() {
  local url="$1"
  local name="$2"
  local i
  for i in $(seq 1 "${WAIT_FOR_HTTP_TRIES:-60}"); do
    if curl -sf "$url" >/dev/null 2>&1; then
      return 0
    fi
    sleep "${WAIT_FOR_HTTP_INTERVAL:-0.5}"
  done
  echo "$name did not become ready at $url after ~$(( ${WAIT_FOR_HTTP_TRIES:-60} / 2 ))s" >&2
  return 1
}
...
bp_machine='{"version":"1.0.0","machine":{"name":"HealthKit Spezi BP Consumer","description":"Consumes normalized SpeziHealthKit blood pressure source","arbiterRule":"PASSTHROUGH","perceptualMapping":{"input":{"offset":4320,"length":4},"output":{"offset":4350,"length":2}},"sequences":[{"id":"healthkit-spezi-bp-seq","name":"BP signal accepted","events":[{"id":"healthkit-spezi-bp-ready","elements":[{"value":0.72,"comparatorType":"equals"},{"value":0.48,"comparatorType":"equals"},{"value":0.24,"comparatorType":"equals"},{"value":0.99,"comparatorType":"equals"}],"isInitial":true,"outputEvents":[{"id":"healthkit-spezi-bp-out","vector":[1,0]}]}]}]}}'
exercise_machine='{"version":"1.0.0","machine":{"name":"HealthKit Spezi Exercise Consumer","description":"Consumes normalized SpeziHealthKit exercise source","arbiterRule":"PASSTHROUGH","perceptualMapping":{"input":{"offset":4330,"length":4},"output":{"offset":4352,"length":2}},"sequences":[{"id":"healthkit-spezi-exercise-seq","name":"Exercise signal accepted","events":[{"id":"healthkit-spezi-exercise-ready","elements":[{"value":0.65,"comparatorType":"equals"},{"value":0.58,"comparatorType":"equals"},{"value":0.42,"comparatorType":"equals"},{"value":0.97,"comparatorType":"equals"}],"isInitial":true,"outputEvents":[{"id":"healthkit-spezi-exercise-out","vector":[1,0]}]}]}]}}'
sleep_machine='{"version":"1.0.0","machine":{"name":"HealthKit Spezi Sleep Consumer","description":"Consumes normalized SpeziHealthKit sleep source","arbiterRule":"PASSTHROUGH","perceptualMapping":{"input":{"offset":4340,"length":4},"output":{"offset":4354,"length":2}},"sequences":[{"id":"healthkit-spezi-sleep-seq","name":"Sleep signal accepted","events":[{"id":"healthkit-spezi-sleep-ready","elements":[{"value":0.82,"comparatorType":"equals"},{"value":0.12,"comparatorType":"equals"},{"value":0.18,"comparatorType":"equals"},{"value":0.96,"comparatorType":"equals"}],"isInitial":true,"outputEvents":[{"id":"healthkit-spezi-sleep-out","vector":[1,0]}]}]}]}}'
